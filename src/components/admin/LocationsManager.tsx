...
const LocationsManager = () => {
  ...
  const load = async () => {
    const { data } = await supabase.from("search_locations").select("*").order("display_order");
  ...
  const addItem = async () => {
    ...
    const { error } = await supabase.from("search_locations").insert({
      name_en: newEn.trim(),
      name_he: newHe.trim(),
  ...
  return (
    <div className="space-y-6">
      <h2 className="font-display font-semibold text-foreground text-lg">Search Locations</h2>
      <div className="flex gap-3 items-end">
        <div className="flex-1">
          <label className="text-xs text-muted-foreground font-body mb-1 block">English</label>
          <input value={newEn} onChange={e => setNewEn(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm font-body bg-card" placeholder="e.g. Zichron Yaakov" />
        </div>
        <div className="flex-1">
          <label className="text-xs text-muted-foreground font-body mb-1 block">Hebrew</label>
          <input value={newHe} onChange={e => setNewHe(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm font-body bg-card" placeholder="e.g. זכרון יעקב" />
        </div>
