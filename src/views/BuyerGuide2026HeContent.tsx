...
const NEIGHBORHOODS = [
  ...
  },
  {
    ...
    cons: ["נדרש רכב", "פחות נוח להליכה מהמרכז ההיסטורי"],
...
const COMPARE_CITIES = [
  ...
  ["זכרון יעקב", "₪27,400 ממוצע", "בוטיק, ים-תיכוני", "15%–20% מהאוכלוסייה"],
    ...
    <div dir="rtl" className="min-h-screen bg-background">
      ...
      <div className="container px-6 max-w-3xl mx-auto py-16 md:py-24">
        ...
        <section>
          ...
          <p className="font-display italic text-lg text-muted-foreground mb-8">מחירים, מגמות ועסקאות אמיתיות <Num>2025–2026</Num></p>
          ...
          <h3 className="font-display text-xl font-semibold text-foreground mt-10 mb-5">מחיר למ״ר לפי אזור (<Num>2025–2026</Num>)</h3>
          <div className="overflow-x-auto my-8">
            <table className="w-full text-sm font-body border border-border rounded-lg overflow-hidden">
            ...
            <table className="w-full text-sm font-body border border-border rounded-lg overflow-hidden">
              ...
              <tbody>
                {COMPARE_CITIES.map(([city, price, life, anglo], i) => (
                  <tr key={i} className={city === "זכרון יעקב" ? "bg-gold/10 font-semibold" : i % 2 === 0 ? "bg-card" : "bg-sand-light"}><td className="p-3">{city}</td><td className="p-3"><Num>{price}</Num></td><td className="p-3 text-muted-foreground">{life}</td><td className="p-3 text-muted-foreground"><Num>{anglo}</Num></td></tr>
                ))}
              </tbody>
            </table>
          </div>
